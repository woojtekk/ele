...
def log_reset():
    ...
    with open(FILE_START, 'w')    as f: f.write("STOP")
...
def switch():
    with open(FILE_SWITCH, 'r') as f:
        ...
        if line == "OFF":
            ...
            with open(FILE_SWITCH, 'w+') as the_file:
                the_file.write("ON")
            return "OFF"
        else:
            return "ON"
...
def start():
    with open(FILE_START, 'r') as f:
        ...
        line = line.rstrip()
        if line == "START":
            logging.info("START measurements freom start funcztion")
            with open(FILE_START, 'w+') as the_file:
                the_file.write("STOP")
            return 1
        else:
            return 0
...
def change():
    ...
    if os.path.isfile(FILE_CHANGE) and os.path.getsize(FILE_CHANGE) > 0:
        with open(FILE_CHANGE, 'r') as f:
            ...
            if lines == 2:
                line = f.readlines()
                if line[0].split("=")[0] == "VGS":
                    VGS = float(line[0].split("=")[1])
                elif line[0].split("=")[0] == "VDS":
                    VDS = float(line[0].split("=")[1])
                if line[1].split("=")[0] == "VGS":
                    VGS = float(line[1].split("=")[1])
                elif line[1].split("=")[0] == "VDS":
                    VDS = float(line[1].split("=")[1])
                else:
                    return False, 0, 0
